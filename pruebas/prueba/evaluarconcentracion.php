...
$folder="../../";
include_once("../../estructurabd/rec_banco_concentracion_respuestas.php");
$rec_banco_concentracion_respuestas=new rec_banco_concentracion_respuestas;
...
if(count($r)>0){
	foreach($r as $nro=>$respuesta){
		$rec_b_c=$rec_banco_concentracion->mostrarTodoRegistro("cod_concentracion='$nro'",0,"");	
		$rec_b_c=array_shift($rec_b_c);
		$correcta=$rec_b_c['respuesta'];
        for($i=1;$i<=31;$i++){
            //array_search(
        }
        $a=array();
		...
		if($sw==0){
			$rec_banco_concentracion_respuestas->insertarRegistro($valores,0);
	...
	//}
    $rec_b=$rec_banco_concentracion->mostrarTodoRegistro("",0,"");	
    $total=count($rec_b);
